...
//		新建一个配适器，用于键新的窗体来装载选择的图形和其关联的图形
		connectionAdapter= new ConnectionAdapter(shell2,context,primary);
//		设置内容，新建菜单栏
	...
	private void createMenuBar(Shell shell) {
		...
		classFind(menuBar);
		returnTypeFind(menuBar);
	}

	private void returnTypeFind(Menu menuBar) {
		MenuItem find = new MenuItem(menuBar, SWT.CASCADE);
//		创建查找菜单，弹出式的窗口，
		find.setText("根据返回值，查找");
...
//				新建窗体，添加文本输入框，和按钮
				...
				Composite composite = new Composite(shell, SWT.NATIVE);
				composite.setLayout(fillLayout);
				text= new Text(composite,SWT.SEARCH|SWT.ICON_SEARCH|SWT.ICON_CANCEL);
				button= new Button(composite,SWT.NATIVE);
				button.setText("button");
				button.setVisible(true);
				text.setVisible(true);
				/*Composite composite2 = new Composite(shell, SWT.NATIVE);
				composite2.setLayout(gridLayout);*/
				StyledText text3 = new StyledText(shell,SWT.MULTI);
				text3.setVisible(true);
				text3.setAlwaysShowScrollBars(true);
				text3.setLayout(rowLayout);
//				添加按钮选择事件
...
//						当按钮选择时，将文本框中的内容读取出来。然后查找出对应的图形
						...
						Context context2 = connectionAdapter.getContext();
						List<ClassDetail_StringMode> classDetail = context2.getClassDetail();
						for (ClassDetail_StringMode classDetail_StringMode : classDetail) {
							ArrayList<MethodDetail> methodDetails = classDetail_StringMode.getMethodDetails();
							for (MethodDetail methodDetail : methodDetails) {
								String returnType = methodDetail.getReturnType();
								if(text2.equals(returnType)) {
									String className = classDetail_StringMode.getClassName();
									String methodName = methodDetail.getMethodName();
									text3.append(className+"-->"+methodName+"   \r\n ");
	...
	private void classFind(final Menu menuBar) {
		MenuItem find = new MenuItem(menuBar, SWT.CASCADE);
//		创建查找菜单，弹出式的窗口，
		find.setText("根据类名，查找");
		find.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
						...
						for (myUmlFigure myUmlFigure : children) {
							...
							if(text2.equalsIgnoreCase(text3)) {
								...
								if(text2.equalsIgnoreCase(text3.substring(indexOf+1, text3.length()))) {
									myUmlFigure.requestFocus();
									ConnectionAdapter.showConnection(myUmlFigure);
								}
							}
						}
					}
